...
function createRecordStream( filePath, dirPath ){
  ...
  const context = fileContext.create(filePath, fileContext.getIdPrefix(filePath, dirPath));
  const contextStream = through.obj((row, _enc, next) => next(null, fileContext.attach(row, context)));
