...
const ADDENDUM_FIELDS = [ 'CITY', 'DISTRICT', 'REGION' ];
  ...
  return through.obj(
    function write( record, enc, next ){
      ...
      try {
        ...
        // additional document metadata which may be useful as an addendum
        addrDoc.setAddendum( 'oa', ADDENDUM_FIELDS.reduce((memo, field) => {
          if( _.isString( record[field] ) ) {
            let value = record[field].trim();
            if( value.length ){
              memo[field.toLowerCase()] = value;
            }
          }
          return memo;
        }, {}));
