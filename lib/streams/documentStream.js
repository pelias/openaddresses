 ...
 */
 function createDocumentStream(id_prefix, stats, useOaAdminFields) {
  /**
  ...
  return through.obj(
    function write( record, enc, next ){
      ...
      try {
        ...
        if (useOaAdminFields) {
          if (record.CITY) {
            addrDoc.parent.locality.push(record.CITY);
          }

          if (record.DISTRICT) {
            addrDoc.parent.county.push(record.DISTRICT);
          }

          if (record.REGION) {
            addrDoc.parent.region.push(record.REGION);
          }     
        }     
