...
const dict = {
  ...
  personalTitles: dictionary('en', 'personal_titles.txt', null, 2),
  highwayTypes: dictionary('en', 'highway_types.txt', null, 2),
  streetPrefixes: dictionary('en', 'street_prefixes.txt', null, 2),
  streetTypes: _.merge(
    dictionary('en', 'street_types_usps.txt', null, 2),
    dictionary('en', 'street_types_overrides.txt', null, 2)
  )
  // streetTypes: _.merge(
  //   dictionary('en', 'street_types_libpostal.txt', null, 2),
  //   dictionary('en', 'street_types_overrides.txt', null, 2)
  // )
};
...
function cleanupStreetName(input) {
  ...
  // street 'prefix' expansion ie. the 'Ft.' or 'Bch.' portion
  tokens = synonyms({
    dictionary: dict.streetPrefixes,
    maxReplacements: 1
  })(tokens);
