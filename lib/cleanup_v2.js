 ...
 * 3. Expand/contract the 'directional' portion of the name
 * 4. Capitalize all lowercased words
 */

// load dictionaries from disk
const dict = {
  directonals: dictionary({
    countryCode: 'en',
    filename: 'directional_conversions.txt',
...
function cleanupStreetName(input) {
  ...
  if (tokens.length >= 2){
    tokens = synonyms({
      ...
      // ignore tokens in the directonals dict
      ignore: (token) => _.has(dict.directonals, _.toLower(token.body))
    })(tokens);
  }

  // directional conversions (leftmost token)
  if (tokens.length >= 3) {
    tokens = synonyms({
      dictionary: dict.directonals,
      maxElements: 1,
      maxReplacements: 1,
  ...
  // directional conversions (rightmost token)
  if (tokens.length >= 3) {
    tokens = synonyms({
      dictionary: dict.directonals,
      maxElements: 1,
