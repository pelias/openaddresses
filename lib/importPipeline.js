 ...
 */
function createFullImportPipeline(files, dirPath, deduplicatorStream, adminLookupStream, useOaAdminFields, finalStream ){ // jshint ignore:line
  logger.info( 'Importing %s files.', files.length );

  finalStream = finalStream || peliasDbclient();

  recordStream.create(files, dirPath, useOaAdminFields)
